...
model.prototype.update = function(props, options, callback){
  ...
  if (callback) {
    ...
    parsed_options = monk.util.options(options);
  ...
  if (parsed_options && parsed_options.new)
  ...
  this.constructor.hooks._call("before", "update", this, function(error){
    ...
    this.constructor.collection[operation]({_id: this._id}, query, options, function (error, updated){
      if (error)
        return promise.fulfill.call(promise, error);
      ...
      promise.fulfill.call(promise, error, to_return);
